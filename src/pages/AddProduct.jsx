...
export default function AddProduct({ onNavigate }) {
  ...
  const linkLoadingSteps = [
    ...
    'Checking if this is clothing...',
    'Fetching product image...',
    'Reading product name and details...',
  ...
  const ensureUser = () => {
    ...
    setErrorMsg('Please log in again before adding wardrobe items.');
    return false;
  };
  ...
  const handleLinkSubmit = async () => {
    ...
    if (!productLink.trim()) {
      setErrorMsg('Please paste a product link.');
      return;
    }
  ...
  if (step === 'form') {
    return (
      <div className="w-full max-w-md mx-auto h-full flex flex-col bg-[#f0f0f0] text-gray-900 scrollbar-hide relative overflow-y-auto">
        {/* Header */}
        <div className="flex items-center gap-3 px-5 pt-12 pb-6">
          <button
            onClick={() => onNavigate('home')}
            className="w-9 h-9 bg-white border border-gray-200 hover:bg-gray-50 rounded-full flex items-center justify-center transition"
          >
          ...
          <div>
            <p className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold">Add Item</p>
            <h2 className="text-lg font-bold text-gray-900 leading-tight">Upload Clothing</h2>
          </div>
        </div>
        ...
        <div className="flex-1 px-5 pb-10 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-2 bg-white shadow-sm border border-gray-200 rounded-2xl p-1.5">
            {[
              ['link', 'Add with link'],
              ['upload', 'Upload photo'],
            ].map(([mode, label]) => (
          ...
          {inputMode === 'link' && (
            <div className="space-y-4">
              <DarkInput
                label="Product Link"
                placeholder="Paste a clothing product URL"
                value={productLink}
  ...
  if (step === 'loading') {
    ...
    const isLink = activeMode === 'link';
    return (
      <div className="w-full max-w-md mx-auto h-full flex flex-col items-center justify-center bg-[#f0f0f0] text-gray-900 scrollbar-hide p-8 text-center">
        {/* Animated ring */}
  ...
  if (step === 'avatar' && avatarResult) {
    ...
    return (
      <div className="w-full max-w-md mx-auto h-full flex flex-col bg-[#f0f0f0] text-gray-900 scrollbar-hide overflow-y-auto">
        {/* Header */}
        <div className="flex items-center gap-3 px-5 pt-12 pb-4">
          ...
          <div>
            <p className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold">AI Generated</p>
            <h2 className="text-lg font-bold text-gray-900 leading-tight">Your Avatar &amp; Bundle</h2>
          </div>
        </div>
        ...
        <div className="px-5 pb-10">
          ...
          <div className="mb-5">
            {avatarImgUrl ? (
              ...
              <div className="w-full h-64 rounded-3xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-center p-6">
                <span className="text-4xl mb-3">🤖</span>
                <p className="text-sm font-semibold text-gray-600">Avatar Generation Failed</p>
          ...
          <div className="flex items-center gap-3 mb-5">
            {/* Compatibility Score */}
            <div className="flex-1 bg-white shadow-sm border border-gray-200 rounded-2xl p-3 text-center">
              <p className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">Compatibility</p>
            ...
            <div className="flex-[2] flex flex-wrap gap-1.5 items-center">
              ...
              {(ar.occasion_tags || []).slice(0, 2).map(tag => (
                <span
                  key={tag}
                  className="bg-white shadow-sm border border-gray-200 text-gray-500 text-[10px] font-bold px-2.5 py-1 rounded-full"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="mb-5">
            ...
            <div className="space-y-2">
              {allBundleItems.map((item, idx) => {
                ...
                const colorClass = categoryColors[item.category] || 'bg-white shadow-sm text-gray-600 border-gray-200';
                return (
                  <div
                    key={idx}
                  ...
                  >
                    {/* Color swatch */}
                    <div
                      className="w-10 h-10 rounded-xl flex-shrink-0 border border-gray-200 flex items-center justify-center"
              ...
              {allBundleItems.length <= 1 && (
                <div className="text-center py-6 bg-white shadow-sm border border-gray-200 rounded-2xl">
                  <p className="text-xs text-gray-400 font-semibold">No matching items found in wardrobe.</p>
                  <p className="text-[10px] text-gray-300 mt-1">Add more items to get a complete bundle.</p>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Dominant Color */}
          {ar.dominant_color && (
            <div className="mb-5 bg-white shadow-sm border border-gray-200 rounded-2xl p-3 flex items-center gap-3">
              <div
                className="w-8 h-8 rounded-xl flex-shrink-0 border border-gray-200"
                style={{ backgroundColor: colorToHex(ar.dominant_color) }}
              />
              <div>
                <p className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">Dominant Palette</p>
                <p className="text-xs font-bold text-gray-900">{ar.dominant_color} · {ar.dominant_palette}</p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3 mt-6">
            ...
            <button
              onClick={handleAvatarGenerate}
              className="w-full py-4 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 bg-white shadow-sm hover:bg-gray-100 border border-gray-200 text-gray-700 hover:text-gray-900 transition-all"
            >
              <span>🔄</span>
              Regenerate Avatar
            </button>
            ...
            <button
              onClick={() => setStep('form')}
              className="w-full py-3.5 text-sm font-semibold text-gray-400 hover:text-gray-600 transition"
            >
              ← Back to Form
            </button>
          </div>
  ...
  if (step === 'preview' && uploadResult) {
    ...
    return (
      <div className="w-full max-w-md mx-auto h-full flex flex-col bg-[#f0f0f0] text-gray-900 scrollbar-hide p-5 overflow-y-auto">
        <div className="flex items-center gap-3 mb-5">
          <button onClick={() => setStep('form')} className="w-9 h-9 bg-white border border-gray-200 hover:bg-gray-50 rounded-full flex items-center justify-center transition">
            <svg className="w-4 h-4 text-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          <div>
            <p className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold">Review</p>
            <h2 className="text-lg font-bold text-gray-900">Verify AI Result</h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div>
            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Original</span>
            <div className="aspect-[4/5] bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden">
          ...
          <div>
            ...
            <div className="aspect-[4/5] bg-white shadow-sm border-2 border-emerald-500/50 rounded-xl overflow-hidden relative">
              <img src={prodUrl} alt="AI Enhanced" className="w-full h-full object-cover" />
              {!isFallback && (
                <div className="absolute top-2 right-2 bg-emerald-500 text-gray-900 text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider">
                  AI
                </div>
              )}
            </div>
        ...
        <div className="border-t border-gray-200 pt-4 space-y-4">
          <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">AI Inferred Attributes</p>
        ...
        <div className="mt-8 flex flex-col gap-3">
          ...
          <button
            ...
            className="w-full py-3.5 text-sm font-bold text-gray-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-2xl transition duration-300"
          >
            Discard &amp; Start Over
          </button>
        </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        onChange={onChange}
        {...rest}
        className="w-full bg-white shadow-sm border border-gray-200 rounded-xl py-3 px-3.5 text-sm font-medium text-gray-900 placeholder-white/20 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 transition"
      />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <select
        value={value}
        onChange={onChange}
        className="w-full bg-white shadow-sm border border-gray-200 rounded-xl py-3 px-3 text-xs font-semibold text-gray-900 focus:outline-none focus:ring-2 focus:ring-violet-500/50 transition appearance-none"
      >
